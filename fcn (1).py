...
class FCN_res101(nn.Module):
    def __init__(self, in_channels=3, num_classes=7, pretrained=True):
        ...
        self.head = nn.Sequential(nn.Dropout(), nn.Conv2d(2048, 128, kernel_size=1, stride=1, padding=0, bias=False),
...
class FCN_res50(nn.Module):
    def __init__(self, in_channels=3, num_classes=7, pretrained=True):
        ...
        self.head = nn.Sequential(torch.nn.Dropout(0.5),
                                  nn.Conv2d(2048, 128, kernel_size=1, stride=1, padding=0, bias=False),
                                  nn.BatchNorm2d(128), nn.ReLU())
